...
namespace ve {
	...
	veModel::veModel(ve::veDevice& device, const veModel::Builder &builder) : veDevice {device}
	{
	...
	std::unique_ptr<veModel> veModel::createModelFromFile(ve::veDevice& device, const std::string& filepath)
	{
		Builder builder{};
	...
	void veModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		void* data;
		vkMapMemory(veDevice.device(), stagingBufferMemory, 0, bufferSize, 0, &data); // creates region of host(cpu) memory mapped to device(gpu) memory and sets data to point to beginning of mapped data range
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize)); //copies vertex data over to gpu
	...
	void veModel::createIndexBuffer(const std::vector<uint32_t>& indices)
	{
		...
		void* data;
		vkMapMemory(veDevice.device(), stagingBufferMemory, 0, bufferSize, 0, &data); // creates region of host(cpu) memory mapped to device(gpu) memory and sets data to point to beginning of mapped data range
		memcpy(data, indices.data(), static_cast<size_t>(bufferSize)); //copies index data over to gpu
		vkUnmapMemory(veDevice.device(), stagingBufferMemory);
