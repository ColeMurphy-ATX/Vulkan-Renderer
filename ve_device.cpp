...
namespace ve {
    ...
    veDevice::veDevice(Window &window) : window{ window } {
        createInstance(); //initializes the vulkan library, connection between this application and vulkan
        setupDebugMessenger(); //sets up validation layers (checks for errors at the cost of performance, removed for release builds)
        createSurface(); //creates surface that relies on GLFW, connection between window and vulkans ability to display results
        pickPhysicalDevice(); //picks the physical graphics device for working with vulkan (GPU)
        createLogicalDevice(); //describes what features of our physcial device we want to use 
